...
# but not the corresponding "worded" methods (intersection,
...
class FrozenKeyedSet(Set):
    ...
    def __init__(self, iterable, key=lambda x: x):
        """Create a FrozenKeyedSet from iterable; key function
...
class KeyedSetTestCase(TestCase):
    ...
    def test_addremove(self):
        ...
        self.assertFalse(hasattr(s1f, 'add') or hasattr(s1f, 'remove') or
                                                    hasattr(s1f, 'discard'))
    ...
    def test_eq_hash(self):
        ...
        self.assertEquals(hash(s3f), hash(FrozenKeyedSet(self.s1,
                                                     key=lambda o: o.name)))
